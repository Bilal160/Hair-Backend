...
export class BusinessAuthController {
    ...
    static async RegisterBusinessUser(req: Request, res: Response) {
        ...
        try {
            ...
            const userExists = await BusinessAuthService.userExists(result.data.email);
            if (userExists) {
                return sendErrorResponse(res, [`Email already registered try with different email`], 400);
            }
            ...
            if (businessPhotos && businessPhotos.length > 0) {
                const uploadedIds = await handlePhotoUpload(
                    Array.isArray(businessPhotos) ? businessPhotos : [businessPhotos], "businessPhotos"
                );
                if (uploadedIds && Array.isArray(uploadedIds)) {
                    businessPhotosIds = (uploadedIds as string[]).map(id => new Types.ObjectId(id));
                }
            }
            ...
            if (businessNICPhoto && businessNICPhoto.length > 0) {
                const uploadedIds = await handlePhotoUpload(
                    Array.isArray(businessNICPhoto) ? businessNICPhoto : [businessNICPhoto], "businessNICPhoto"
                );
                if (uploadedIds && Array.isArray(uploadedIds)) {
                    businessNICPhotoIds = (uploadedIds as string[]).map(id => new Types.ObjectId(id));
                }
            }
            if (businessRegistrationDoc && businessRegistrationDoc.length > 0) {
                const uploadedIds = await handlePhotoUpload(
                    Array.isArray(businessRegistrationDoc) ? businessRegistrationDoc : [businessRegistrationDoc], "businessRegistrationDoc"
                );
            ...
            if (businessFeaturedImage && businessFeaturedImage.length > 0) {
                const uploadedIds = await handlePhotoUpload(
                    Array.isArray(businessFeaturedImage) ? businessFeaturedImage : [businessFeaturedImage], "businessFeaturedImage"
                );
                if (uploadedIds && Array.isArray(uploadedIds) && uploadedIds.length > 0) {
            ...
            const businessProfile = await BusinessProfileService.createBusinessProfile({
